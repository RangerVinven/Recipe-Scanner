...
from models.tables import recipes_table, steps_table
from models.steps import CreateSteps, UpdateSteps
...
@steps_router.get("/{recipe_id}")
def get_steps(recipe_id: int, request: Request):
    user_id = get_user_id(request)

    # Runs the select query
    recipes_steps_join = join_tables_query(steps_table, steps_table.c.Recipe_ID, recipes_table, recipes_table.c.Recipe_ID)
    select_query = select(steps_table).select_from(recipes_steps_join).where(recipes_table.c.Recipe_ID == recipe_id).where(recipes_table.c.User_ID == user_id)
    results = connection.execute(select_query).mappings().all() # mappings... turns the result into JSON

    return results

@steps_router.post("/{recipe_id}")
...
def delete_step(step_id: int, request: Request):
    user_id = get_user_id(request)

    # Gets the recipe_id to be able to check if the user owns the recipe
    select_query = select(steps_table).where(steps_table.c.Step_ID == step_id)
    ...
    print(result)
    authorise_request(user_id, result[3])

    # Deletes the step
    delete_query = delete(steps_table).where(steps_table.c.Step_ID == step_id)
